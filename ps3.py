...
def is_valid_word(word, hand, word_list):
    ...
    if "*" not in word:
        ...
        for i in word:
            if i in check_hand:
                check_hand = update_hand(check_hand, i)
            else:
                return False
        return True
    else:
        for letter in VOWELS:
            if letter not in check_hand:
                ...
                check_hand[letter] += 1
            if is_valid_word(word.replace("*", letter), check_hand, word_list):
                return True
    return False
        ...
        if len(hand) != 0:
            ...
            if word == "!!":
                ...
                if is_valid_word(word, hand, word_list):
                    ...
                    print(f"“{word}” earned {get_word_score(word, calculate_handlen(hand))}. Total: {total_score}")
                else:
...
def substitute_hand(hand, letter):
    ...
    if letter in hand:
        while 1:
            substitute = random.choice(VOWELS + CONSONANTS)
            if substitute not in hand:
                sub_hand[substitute] = sub_hand[letter]
                del sub_hand[letter]
                break
    return sub_hand
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    while total_hands > 0:
        ...
        scores = []
        while 1:
            scores.append(play_hand(hand, word_list))
            if attempt != 0:
                if input("\nWould you like to replay the hand? ").lower() == "yes":
                    print()
                    attempt -=1
                    continue
                else:
                ...
                break
        hands_score += max(scores)
        total_hands -= 1
    
    print("-"*25)
    print(f"Total score over all hands: {hands_score}")
